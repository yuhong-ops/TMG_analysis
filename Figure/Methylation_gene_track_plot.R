...
tracks <- list(iTrack, gTrack, rTrack, methTrack_tcga, methTrack_gse)
tracks <- list(iTrack, gTrack, rTrack, methTrack_tcga)

sizes <- c(2,2,4,12,12) # set up the relative sizes of the tracks
...
## TTN
# the index of the DMR that we will plot 
which(results.ranges$overlapping.genes=="TTN") # 5488

# 25193, 25610
dmrIndex <- 25193
results.ranges[which(results.ranges$overlapping.genes=="TTN"),]

# extract chromosome number and location from DMR results 
chrom <- as.character(seqnames(results.ranges[dmrIndex]))
start <-  179672121 #as.numeric(start(results.ranges[dmrIndex]))
end <-  179672691 #as.numeric(end(results.ranges[dmrIndex]))
# add 25% extra space to plot
minbase <- start - (0.25*(end-start))
maxbase <- end + (0.25*(end-start))
...
library(TxDb.Hsapiens.UCSC.hg19.knownGene)
txdb <- TxDb.Hsapiens.UCSC.hg19.knownGene
rTrack <- GeneRegionTrack(txdb, chromosome = chrom, 
                          start = minbase,  end = maxbase)

##Ensure that the methylation data is ordered by chromosome and base position.

myDMP_tcga_GABRA5<-myDMP1 %>% dplyr::filter(gene == "TTN") %>% 
  dplyr:: filter ((MAPINFO < maxbase) & (MAPINFO > minbase))

myDMP_gse_GABRA5<-myDMP_gse$no_to_yes %>% dplyr::filter(gene == "TTN") %>% 
  dplyr:: filter ((MAPINFO < maxbase) & (MAPINFO > minbase))
...
# create genomic ranges object from methylaton data
cpgData <- GRanges(seqnames=Rle(rep(chrom,3)),  ##改一下
                   ranges=IRanges(start=myDMP_gse_GABRA5$MAPINFO, end=myDMP_gse_GABRA5$MAPINFO),
                   strand=Rle(rep("*",nrow(myDMP_gse_GABRA5))),
                   betas=GABRA5_beta_gse)


cpgData_tcga <- GRanges(seqnames=Rle(rep(chrom,4)),  ##改一下
                        ranges=IRanges(start=myDMP_tcga_GABRA5$MAPINFO, end=myDMP_tcga_GABRA5$MAPINFO),
                        strand=Rle(rep("*",nrow(myDMP_tcga_GABRA5))),
                        betas=GABRA5_beta_tcga)
...
# methylation data track
methTrack_gse <- DataTrack(range=cpgData, groups=meta_data_thymoma_kajiura$Myasthenia_gravis,genome = gen,
                           chromosome=chrom, ylim=c(0,1), col=pal,
                           type=c("a","p"), name="DNA Meth.\n(beta value)",
                           background.panel="white", legend=TRUE, cex.title=0.8,
                           cex.axis=0.8, cex.legend=0.8)
...
sizes <- c(2,2,4,12,12) # set up the relative sizes of the tracks
plotTracks(tracks, from=minbase-200, to=maxbase, showTitle=TRUE, add53=TRUE, 
           add35=TRUE, grid=TRUE, lty.grid=3, sizes = sizes, length(tracks))

plotTracks(tracks, from=179389770, to=maxbase, showTitle=TRUE, add53=TRUE, 
           add35=TRUE, grid=TRUE, lty.grid=3, sizes = sizes, length(tracks))

## RYR1
# the index of the DMR that we will plot 
which(results.ranges$overlapping.genes=="RYR1") # 5488

# 3162  4564  7740 14732 16180 16999 18769 22109 22231 24791:   选择16999
dmrIndex <- 16999
results.ranges[which(results.ranges$overlapping.genes=="RYR1"),]

# extract chromosome number and location from DMR results 
chrom <- as.character(seqnames(results.ranges[dmrIndex]))
start <-  38924239 #as.numeric(start(results.ranges[dmrIndex]))
end <-  38924519 #as.numeric(end(results.ranges[dmrIndex]))
# add 25% extra space to plot
minbase <- start - (0.25*(end-start))
maxbase <- end + (0.25*(end-start))
...
myDMP_tcga_GABRA5<-myDMP1 %>% dplyr::filter(gene == "RYR1") %>% 
  dplyr:: filter ((MAPINFO < maxbase) & (MAPINFO > minbase))

myDMP_gse_GABRA5<-myDMP_gse$no_to_yes %>% dplyr::filter(gene == "RYR1") %>% 
  dplyr:: filter ((MAPINFO < maxbase) & (MAPINFO > minbase))
...
# create genomic ranges object from methylaton data
cpgData <- GRanges(seqnames=Rle(rep(chrom,8)),  ##改一下
                   ranges=IRanges(start=myDMP_gse_GABRA5$MAPINFO, end=myDMP_gse_GABRA5$MAPINFO),
                   strand=Rle(rep("*",nrow(myDMP_gse_GABRA5))),
                   betas=GABRA5_beta_gse)


cpgData_tcga <- GRanges(seqnames=Rle(rep(chrom,8)),  ##改一下
                        ranges=IRanges(start=myDMP_tcga_GABRA5$MAPINFO, end=myDMP_tcga_GABRA5$MAPINFO),
                        strand=Rle(rep("*",nrow(myDMP_tcga_GABRA5))),
                        betas=GABRA5_beta_tcga)
...
sizes <- c(2,2,4,15,15) # set up the relative sizes of the tracks
